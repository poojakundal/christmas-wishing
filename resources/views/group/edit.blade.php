        ...
        <div class="card-body">
            <form method="POST" action="{{ route('group.update', ['group' => $group->id]) }}" enctype='multipart/form-data'>
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-sm-10">
                        <input type="text" name="name" class="form-control @error('name') is-invalid @enderror" id="name" placeholder="Name" value="{{ $group->name }}" autocomplete="name" autofocus />
                        @error('name')
                    ...
                    <div class="col-sm-10">
                        <select id="users" name="users[]" multiple>
                            @foreach($userLists as $i => $userList)
                                <option value="{{ $userList->id }}" {{ in_array($userList->id,$groupUser) ? 'selected' : '' }}>{{ $userList->name }}</option>
                            @endforeach
                        </select>
